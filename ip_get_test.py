...
def deviceOnlineStateCheck(devIp,PingPacksNum=10):
...
def getPingSuccesIpOnComputerNetCard(Dev_ip,dev_mask):
    ...
    if len(ipMaskInterface_info_arr)==3:#probably find in permanent route list,but alse may in active route list(have chinese word in gateway column)
        if '0.0.0.0' in ipMaskInterface_info_arr:
            print('can not find valid ip,drop to NetGate')
            if '0.0.0.0' in ipMaskInterface_info_arr[1]:
                return ipMaskInterface_info_arr[3]
            return getSameSegmentIpOnComputerNetCard(ipMaskInterface_info_arr[2],ipMaskInterface_info_arr[1])
        return getSameSegmentIpOnComputerNetCard(ipMaskInterface_info_arr[2],ipMaskInterface_info_arr[1])
        ...
        if '0.0.0.0' in ipMaskInterface_info_arr:#0.0.0.0,retrun interface
            ...
            if '0.0.0.0' in ipMaskInterface_info_arr[1]:
                return ipMaskInterface_info_arr[3]
            print(ipMaskInterface_info_arr[2],ipMaskInterface_info_arr[1])
            return getSameSegmentIpOnComputerNetCard(ipMaskInterface_info_arr[2],ipMaskInterface_info_arr[1])
        return getSameSegmentIpOnComputerNetCard(ipMaskInterface_info_arr[2],ipMaskInterface_info_arr[1])
    else:
...
if __name__=='__main__':
    ...
    a=getPingSuccesIpOnComputerNetCard('172.16.66.182','255.255.255.0')
    a1=getPingSuccesIpOnComputerNetCard('172.16.66.183','255.255.255.0')
    print(a,a1)