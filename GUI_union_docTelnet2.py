...
class Application(Frame):
    ...
    def DownloadFileThroughTelnet(self):
        ...
        if (len(DevList_clear)>0)and(len(CardList_clear)>0):
            ...
            myTask3=threading.Thread(target=telnetDownload.downloadSvcfileMulti_forGUI,args=(DevList_clear,CardList_clear,FtpSet,resetAllOption,))
    ...
    def UpLoadDevConfig(self):
        ...
        for DevList_clear_rawi in DevList_clear_raw:
            if DevList_clear_rawi:
                DevList_clear.append(DevList_clear_rawi)
        finish_message=telnetDownload.upLoadConfigMutil_forGUI(DevList_clear)
        tkMessageBox.showinfo('Message',finish_message)
    ...
    def openUpgradeLogDir(self):
        os.system('start D:\py_ftp_download\upgrade_log\\')
...
#close
    ...
    def loadCardList(self):
        if os.path.isfile(self.conf_fresh_card):
            ...
            tkMessageBox.showinfo('Message',u'未找到配置:'+self.conf_fresh_card)


    def createWidgets(self):
        self.Menubar=Menu(self)
        self.fileMenu=Menu(self.Menubar,tearoff=0)
        self.Menubar.add_cascade(label=u'工具配置保存',menu=self.fileMenu)
        ...
        self.Menubar.add_cascade(label='工具配置加载',menu=self.EditMenu)
        ...
        self.Menubar.add_cascade(label=u'常用工具箱',menu=self.toolsMenu)
